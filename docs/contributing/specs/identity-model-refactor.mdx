...
#### PR 3.1 — Backend exchange endpoint (Entra only)

Backend-only, Entra-path-only. Delivers the endpoint but doesn't wire the frontend to it yet — the endpoint runs dark until PR 3.2.
...
- Config shape: `SecuritySettings:Providers:Entra:{Authority, Audience, AllowedTenantIds, ClockSkewSeconds}`. Nested under `Providers` so Auth0 and other OIDC providers slot in at the same level without a config migration.
- The existing `AzureAd` middleware-based validation stays in place — the frontend still uses MSAL-direct-to-API until PR 3.2 cuts over. Removed in a later PR once the old flow is unused.

#### PR 3.2 — Frontend cutover
