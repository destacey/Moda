...
`PendingMigrationProviderId` is set on `ApplicationUser` to the target provider's `Name`. During token exchange for any OIDC provider, `UserService.ResolveFromGenericOidcPrincipalAsync` (GenericOidc path) or the Entra path checks for a pending provider migration after the normal identity lookup fails. The rebind matches a user by `PendingMigrationProviderId == providerName` AND `NormalizedEmail == token.email`. On match, inside one transaction:
