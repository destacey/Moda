...
description: Planning intervals, iterations, sprint mapping, objectives, predictability, and plan review in Moda.
sidebar_position: 1
audience: [user, agent]
---
...
A **Planning Interval** (PI) is a fixed timeframe — typically 8-12 weeks — during which a group of teams delivers value toward committed objectives. It is the primary unit of scaled planning in Moda.

Every planning interval has:
- **Name** and optional **Description**
- **Date Range** — Start and end date for the PI
- **Teams** — The [teams](../organizations/index#teams) and [teams of teams](../organizations/index#teams-of-teams) participating
...
The PI detail page is the command center for a planning interval. It provides several views:

**Details Tab:**
- Start/End dates and Objectives Locked status
- PI Description (Markdown)
- **Iteration cards** — Each iteration is shown as a clickable card. Click an iteration to navigate to its detail page, where you can see the aggregated sprint backlog and metrics across all participating teams.
- **Metrics and Charts:**
  - **Days Countdown** — Days remaining in the PI (based on state)
  - **PI [Predictability](#predictability)** — Percentage of committed (non-stretch) objectives completed
  - **Team Predictability Radar Chart** — Multi-axis radar showing each team's predictability score
  - **Objective Status Chart** — Pie chart grouping objectives by status
  - **Objective Health Chart** — Pie chart grouping objectives by health check status

**Teams Tab:**
- Grid of all [teams](../organizations/index#teams) assigned to the PI

**Sprint Mappings Tab:**
- Configuration grid mapping each team's [sprints](./sprints) to PI iterations
- Each team can have one sprint per iteration

**Horizontal navigation** across the PI provides quick access to:
- PI Details
...
- **Aggregate Metrics** (for Active/Completed iterations, see [Sprint Metrics](./sprints#sprint-metrics) for definitions):
...
**Backlog Tab:**
- Combined sprint backlog grid showing [work items](../work-management/index#work-items) from all mapped sprints
- Columns: Rank, Key, Type, Title, Story Points, Status, Status Category, Team

> **Tip:** Click on an iteration card on the PI Details tab to navigate directly to its detail page.
...
Team [Sprints](./sprints) are mapped to PI iterations. This mapping connects each team's delivery cadence to the broader PI schedule, enabling aggregate views and cross-team coordination.
...
Objectives bridge the gap between strategic intent and execution. A team commits to objectives during PI planning, then links [work items](../work-management/index#work-items) that fulfill those objectives. As work items are completed in [sprints](./sprints), objective progress is tracked.

Each objective has:
- **Team** — The [team](../organizations/index#teams) committing to the objective
...
- **Work Items Card** — Grid of linked [work items](../work-management/index#work-items) with ability to link/unlink
...
Moda calculates **Predictability** for each PI — the percentage of committed (non-stretch) objectives that were completed.
...
Predictability is only calculated for Active or Completed PIs (not Future). It is shown on the PI Details tab as a metric and visualized per-team in the **Team Predictability Radar Chart**.
...
- Two view modes:
  - **List View** — Objective cards and [risk](./risks) cards for the team
...
3. Review each team's objectives and [risks](./risks) tab-by-tab
