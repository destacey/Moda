...
import { CreateOrUpdateRoleRequest, UpdateRolePermissionsRequest } from "./moda-api";
...
export const useCreateRoleMutation = (queryClient: QueryClient) => {
    return useMutation({
        mutationFn: async (params: CreateOrUpdateRoleRequest) => (await getRolesClient()).create(params),
        onSuccess: (data) => {
...
export const useUpdatePermissionsMutation = (queryClient: QueryClient) => {
    return useMutation({
        mutationFn: async (params: UpdateRolePermissionsRequest) => (await getRolesClient())
            .updatePermissions(params.roleId, { roleId: params.roleId, permissions: params.permissions }),
