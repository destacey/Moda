...
const initialize = async () => {
  ...
  if (msalState.initializing === null) {
    msalInstance.addEventCallback((event) => {
      msalState.interactionStatus = EventMessageUtils.getInteractionStatusFromEvent(event, msalState.interactionStatus) ?? InteractionStatus.None
    })
    msalState.initializing = msalInstance.initialize()
      .then(() => {
