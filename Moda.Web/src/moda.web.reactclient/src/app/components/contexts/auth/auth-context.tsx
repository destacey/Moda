...
const AuthProvider = ({ children }) => {
  const [user, setUser] = useLocalStorageState<User>("current-user", {name: "", username: "", isAuthenticated: false, claims: []})
  ...
  const refreshUser = async () => {
    ...
    if(activeAccount) {
      ...
      const decodedClaims = jwt_decode(accessToken ?? '') as { [key: string]: string }
      const claims: Claim[] = [
      ...
      setUser({name: activeAccount.name, username: activeAccount.username, isAuthenticated: true, claims})
    }
  }

  const hasClaim = (claimType: string, claimValue: string): boolean => {
    return user.claims.some(claim => claim.type === claimType && claim.value === claimValue) ?? false
  }
