...
const TeamMembershipsGrid = ({ teamMemberships }: TeamMembershipsGridProps) => {
    ...
    const columnDefs = useMemo(
        () => [
            ...
            { field: "start", valueGetter: (params) => new Date(params.data.start).toUTCString() },
            { field: "end", valueGetter: (params) => params.data.end ? new Date(params.data.end).toUTCString() : null},
        ],
        []
    );
