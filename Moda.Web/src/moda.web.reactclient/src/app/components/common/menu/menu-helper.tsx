...
export const menuItem = (display: string, key: string, route?: string, icon?: React.ReactNode, children?: Item[]) => {
  return {
      display, key, route, icon, children
  }
}

export const restrictedMenuItem = (authClaimValue: string, authClaimType:string, display: string, key: string, route?: 
  string, icon?: React.ReactNode, children?: Item[]) => {
  return {
  ...
  if ('type' in item) {
      ...
      if (claimCheck(item.authClaimType, item.authClaimValue)) {
          const children = item.children 
              ? item.children.reduce((acc, item) => filterAndTransformMenuItem(acc, item, claimCheck), [])
              : undefined;
